...
def remove_outliers_iqr(df, k=1.5): # Changing it to 2 from 1.5 | 1.618
...
random_int = randrange(30)

print ("Establishing Prediction")
prediction = xg_reg.predict(X_val)
#prediction = xg_reg.predict(test_features)


single_dataframe = pd.DataFrame(X_val.iloc[random_int]).T
single_prediction = xg_reg.predict(single_dataframe)

print ('Single Prediction: ', single_prediction)
print ('Actual Value: ', y_val.iloc[random_int])

#print (test_features.columns.tolist())

def mean_squared_log_error(test_labels, prediction):
    # Ensure non-negative predictions and actual values

    #prediction = [0.001 if a_ == 0 else a_ for a_ in prediction]
    #test_labels = [0.001 if a_ == 0 else a_ for a_ in test_labels]

    prediction = np.maximum(0.00001, prediction)
    test_labels = np.maximum(0.00001, test_labels)
    
    # Calculate the squared logarithmic error
    msle = np.mean((np.log1p(test_labels) - np.log1p(prediction)) ** 2)
    return msle

msle_value = mean_squared_log_error(y_val, prediction)
print("MSLE:", msle_value)

def symmetric_mean_absolute_percentage_error(test_labels, prediction):
    ...
    diff = np.abs(y_true - y_pred) / (denominator / 2)
    smape = np.mean(diff) * 100  # Convert to percentage
    return smape

def root_mean_square_percentage_error(test_labels, prediction):
    ...
    diff_pct = (np.abs(test_labels - prediction) / (np.abs(test_labels) + epsilon)) * 100
    diff_sqrd = diff_pct**2
    diff_mean = np.mean(diff_sqrd)
    rmspe = np.sqrt(diff_mean)

    return rmspe

# Bringing back to normalized space
#y_val = y_val/100.0
#prediction = prediction/100.0

# Example usage
smape_value = symmetric_mean_absolute_percentage_error(y_val, prediction)
...
print ('MAPE: ', round(percentage_error, 3))

rmspe = root_mean_square_percentage_error(y_val, prediction)
...
plt.scatter(y_val, prediction, c=HEX)
plt.xlabel('Actual (max_return)')
plt.ylabel('Predicted (max_return)')
lims = [-10, 10]
plt.xlim(lims)
plt.ylim(lims)
...
'''
# Printing without log prediction

y_val_normalized = [math.exp(value) - 100 for value in y_val]
prediction_normalized = [math.exp(value) - 100 for value in prediction]

plt.scatter(y_val_normalized, prediction_normalized)
plt.xlabel('Normalized Actual (max_return)')
plt.ylabel('Normalized Predicted (max_return)')
lims = [-40, 75]
plt.xlim(lims)
plt.ylim(lims)
_ = plt.plot(lims, lims)
...
smape_value = symmetric_mean_absolute_percentage_error(y_val_normalized, prediction_normalized)
print("SMAPE:", smape_value)


rmse = np.sqrt(mean_squared_error(y_val_normalized, prediction_normalized))

print ('RMSE: ', round(rmse,3))

percentage_error = mean_absolute_percentage_error(y_val_normalized, prediction_normalized)

print ('MAPE: ', round(percentage_error, 3))

rmspe = root_mean_square_percentage_error(y_val_normalized, prediction_normalized)
...
#print(*train_features.columns[sorted_indices], sep = "\n")

xgb.plot_importance(xg_reg, max_num_features=90)
